...
-- mysql's bool type is really a tinyint(1), so we use smallint here.

BEGIN;

DROP TYPE IF EXISTS wlc_state CASCADE;
DROP SEQUENCE IF EXISTS wikilog_comments_wlc_id CASCADE;
...
CREATE TABLE /*$wgDBprefix*/wikilog_wikilogs (
  ...
  wlw_page INTEGER NOT NULL,  -- REFERENCES page(page_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED
...
CREATE TABLE /*$wgDBprefix*/wikilog_posts (
  ...
  wlp_page INTEGER NOT NULL,  -- REFERENCES page(page_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED

  -- Parent wikilog.
  wlp_parent INTEGER NOT NULL,  -- REFERENCES wikilog_wikilogs(wlw_page) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED
  ...
  wlp_publish SMALLINT NOT NULL DEFAULT 0,
...
CREATE TABLE /*$wgDBprefix*/wikilog_authors (
  ...
  wla_page INTEGER NOT NULL,  -- REFERENCES page(page_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED

  -- ID of the author of the post.
  wla_author INTEGER NOT NULL,  -- REFERENCES mwuser(user_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED

  -- Name of the author of the post.
  wla_author_text TEXT NOT NULL,
...
CREATE TABLE /*$wgDBprefix*/wikilog_tags (
  ...
  wlt_page INTEGER,  -- REFERENCES page(page_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED

  -- Tag associated with the post.
  wlt_tag TEXT NOT NULL,
...
CREATE TABLE /*$wgDBprefix*/wikilog_comments (
  ...
  wlc_parent INTEGER,  -- REFERENCES wikilog_comments(wlc_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED
  ...
  wlc_thread TEXT NOT NULL DEFAULT '',

  -- Reference to post wiki article which this comment is associated to.
  wlc_post INTEGER NOT NULL,  -- REFERENCES page(page_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED

  -- ID of the author of the comment, if a registered user.
  wlc_user INTEGER NOT NULL,  -- REFERENCES mwuser(user_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED

  -- Name of the author of the comment.
  wlc_user_text TEXT NOT NULL,
  ...
  wlc_status wlc_state NOT NULL DEFAULT 'OK',
  ...
  wlc_comment_page INTEGER,  -- REFERENCES page(page_id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED
